...
from internal.core.dependencies import ManagerBaseAuthorize, EmployeeBaseAuthorize
...
@DOCUMENT_ROUTER.get('/document/types', dependencies=[Depends(EmployeeBaseAuthorize)])
async def get_doctypes_handler(service: DocumentService = Depends()):
    data = await service.get_document_types()
    return data


@DOCUMENT_ROUTER.post('/document/type', dependencies=[Depends(ManagerBaseAuthorize)])
...
@DOCUMENT_ROUTER.post('/filter_documents', dependencies=[Depends(EmployeeBaseAuthorize)])
...
@DOCUMENT_ROUTER.post('/document', dependencies=[Depends(ManagerBaseAuthorize)])
...
@DOCUMENT_ROUTER.patch('/document', dependencies=[Depends(ManagerBaseAuthorize)])
